...
def train():
    ...
    with mlflow.start_run() as run:
        ...
        # Register model manually via MlflowClient (this triggers model folder)
        client = MlflowClient()
        model_uri = f"runs:/{run.info.run_id}/model"
        mlflow.sklearn.log_model(stack_model, artifact_path="model")
        model_details = client.create_registered_model("fraud_stack_model")

        # Register the current run into the registered model
